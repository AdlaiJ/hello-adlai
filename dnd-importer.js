...
(() => {
  ...
  renderPdfs=function(){
    ...
    $('#pdfLibrary').innerHTML=pdfs.filter(p=>p.name.toLowerCase().includes(query)).map(p=>`<div class="pdf-chip ${p.open?'open':''}" data-pdf="${p.id}"><span>${esc(p.name)}</span><button data-action="toggle" title="${p.open?'Close':'Open'} ${esc(p.name)}">${p.open?'◉':'○'}</button>${p.kind==='statblock'?'':`<button data-action="split" title="Split into statblocks">Split</button>`}<button data-action="delete" title="Delete ${esc(p.name)}">×</button></div>`).join('');
  ...
  function renderDetail(){
    ...
    $('#splitDetail').innerHTML=`<label><input id="splitInclude" type="checkbox" ${b.selected?'checked':''}> Include this statblock</label><input id="splitName" class="resource-input" aria-label="Statblock name" type="text" value="${esc(b.name)}"><p class="split-note">${esc(b.notes)} Bounds are percentages of the source page. A statblock can have multiple regions from the same or different pages.</p><div id="splitRegions">${b.segments.map((s,i)=>`<fieldset data-segment="${i}"><legend>Region ${i+1}</legend><div class="crop-fields">${['page','x','y','w','h'].map(k=>`<label>${{page:'Page',x:'Left %',y:'Top %',w:'Width %',h:'Height %'}[k]}<input aria-label="Region ${i+1} ${{page:'page',x:'left',y:'top',w:'width',h:'height'}[k]}" data-field="${k}" type="number" min="${k==='page'?1:0}" max="${k==='page'?review.doc.numPages:100}" step="${k==='page'?1:.1}" value="${k==='page'?s[k]:+(s[k]*100).toFixed(2)}"></label>`).join('')}<button class="btn small" data-remove="${i}">Remove region</button><a class="back" target="_blank" href="${review.url}#page=${s.page}">View full source page</a></div></fieldset>`).join('')}</div><button class="btn small" id="splitRegionAdd">+ Continuation region</button><div class="crop-preview" id="splitPreview" style="margin-top:12px"></div>`;
